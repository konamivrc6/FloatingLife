"""Markdown 排版标准化：备份 → 标准化 → 写回。正文排版的唯一权威。

全部步骤只作用于 fenced 代码块之外：每个行首围栏块（``` / ~~~，含围栏行本身与 info string）在管线开始前被整体遮成一个哨兵字符，跑完逐字还原，所以代码块内容逐字节原样保留。

用法：python _tool_fmt_md.py [文件...] [-all]；-all 递归处理当前目录下所有未被 .toolignore 排除的 .md 且跳过确认；不给参数则同样递归，但先询问一次确认。
...
# ── 跨步骤占位符：mask_deferred_placeholders 写入，restore_placeholders 统一还原 ──
...
# eat_space=True：step_cjk_ascii_space 会在占位符与中文之间插一个空格，原文本身也可能带一个，还原时吃掉左右各至多一个。吃空格是输出契约的一部分，不是顺手清理，不可改成不吃。
# eat_space=False 的两个注释占位符原样还原，不可改成 True。
# token 里不可出现连续两个下划线：step_underscore 的 (?<!_)_(?!_) 保护不到 __，会被转义成 \_\_ 而还原失败。DOT 系的单下划线能活过那一步，是因为它先被换成下划线占位符、随后又换回来——改名只要不含 __ 就安全。
PLACEHOLDERS = {
...
# ── 步骤内临时占位符：只在一个步骤内生存，用完立即还原，不参与跨步骤还原 ──
...
# BMP 私有区单字符：非 ASCII、非 CJK、非标点，各步骤的正则与 process_paired_markers 的字符扫描都碰不到它。围栏边界上原先只可能落反引号或换行，两者也不在 HW / CJK 类里，所以在此处放哨兵不会凭空造出可排版的位置。
# 用固定哨兵 + 按位置还原，不要用 chr(0xE000 + i)：索引数字是 ASCII 且落在 HW 类里，一旦与中文相邻就会被插空格；索引一大还会走出私有区撞进 CJK 区间。
CODE_SENTINEL = '\uE000'
...
# 匹配范围不含开头行之前的换行、也不含闭合行之后的换行——否则会吃掉围栏与相邻空行之间的换行，让 step_blank_lines 的「拆空行再翻倍」在围栏两侧得出错误结果。
...
def assert_placeholders_absent(text, filepath):
    ...
    一并覆盖哨兵：源文件若已含 U+E000，无围栏时 unmask 会 StopIteration，有围栏时则静默还原成错误的块；前置检查把这两种情况变成一条清晰报错。
    """
...
def step_hygiene(text):
    """删除 <br/> 与零宽空格。跑在掩码之后，所以围栏里的它们会被保留。"""
    return re.sub('<br/>|\u200B', '', text)


def mask_deferred_placeholders(text):
    """把 DOT span 的两个标签藏成占位符，交给 restore_placeholders 统一还原。"""
...
def step_blank_lines(text):
    """空行标准化：删行尾空格、压缩连续空行、把三连空行展开成 <br/> 段、保护表格行、合并引用行。
...
def format_markdown(text):
    ...
    # 行尾统一 LF。\r 算文件卫生而非排版，所以放在掩码之前、全局生效；否则围栏内的 \r 会被原样还原，CRLF 输入变成「栏外 LF、栏内 CRLF」的混合行尾。
    text = text.replace('\r', '')
...
def main():
    args = sys.argv[1:]
    # -all：递归全部并跳过确认，供脚本调用；同传的位置参数被忽略，「全部」已涵盖它们
